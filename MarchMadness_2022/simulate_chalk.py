...
def sim_game(team1, team2, round, points_dict, force):
    ...
    if(len(force) == 0):
        ...
        if(team1_win_percentage > team2_win_percentage):
            ...
            winner_percentage = team1_win_percentage * 100
            loser_percentage = team2_win_percentage * 100
        else:
            winner = team2
            loser = team1

            winner_percentage = team2_win_percentage * 100
            loser_percentage = team1_win_percentage * 100
    ...
    print(f"{winner.seed} {winner.name} ({winner_percentage:.2f}%)  beats {loser.seed} {loser.name} ({loser_percentage:.2f}%)")
...
def sim_tournament():
    ...
    # First Four
    # 2022
    mw_firstfour_16 = sim_game(texas_southern, texas_am_corpus_christi, -1, points_dict, [])
    west_firstfour_11 = sim_game(rutgers, notre_dame, -1, points_dict, [])
    east_firstfour_12 = sim_game(wyoming, indiana, -1, points_dict, [])
    south_firstfour_16 = sim_game(wright_state, bryant, -1, points_dict, [])

    #####  West Region  #####
    # Round of 64
    # 2022
    r64_w_game1 = sim_game(gonzaga, georgia_state, 64, points_dict, [])
    r64_w_game2 = sim_game(boise_state, memphis, 64, points_dict, [])
    r64_w_game3 = sim_game(connecticut, new_mexico_state, 64, points_dict, [])
    r64_w_game4 = sim_game(arkansas, vermont, 64, points_dict, [])
    r64_w_game5 = sim_game(alabama, west_firstfour_11, 64, points_dict, [])
    r64_w_game6 = sim_game(texas_tech, montana_state, 64, points_dict, [])
    r64_w_game7 = sim_game(michigan_state, davidson, 64, points_dict, [])
    r64_w_game8 = sim_game(duke, csu_fullerton, 64, points_dict, [])
    ...
    # Round of 64
    # 2022
    r64_mw_game1 = sim_game(kansas, mw_firstfour_16, 64, points_dict, [])
    r64_mw_game2 = sim_game(san_diego_state, creighton, 64, points_dict, [])
    r64_mw_game3 = sim_game(iowa, richmond, 64, points_dict, [])
    r64_mw_game4 = sim_game(providence, south_dakota_state, 64, points_dict, [])
    r64_mw_game5 = sim_game(lsu, iowa_state, 64, points_dict, [])
    r64_mw_game6 = sim_game(wisconsin, colgate, 64, points_dict, [])
    r64_mw_game7 = sim_game(usc, miami, 64, points_dict, [])
    r64_mw_game8 = sim_game(auburn, jacksonville_state, 64, points_dict, [])
    ...
    # Round of 64
    # 2022
    r64_e_game1 = sim_game(baylor, norfolk_state, 64, points_dict, [])
    r64_e_game2 = sim_game(north_carolina, marquette, 64, points_dict, [])
    r64_e_game3 = sim_game(saint_marys, east_firstfour_12, 64, points_dict, [])
    r64_e_game4 = sim_game(ucla, akron, 64, points_dict, [])
    r64_e_game5 = sim_game(texas, virginia_tech, 64, points_dict, [])
    r64_e_game6 = sim_game(purdue, yale, 64, points_dict, [])
    r64_e_game7 = sim_game(murray_state, san_francisco, 64, points_dict, [])
    r64_e_game8 = sim_game(kentucky, st_peters, 64, points_dict, [])
    ...
    # Round of 64
    # 2022
    r64_s_game1 = sim_game(arizona, south_firstfour_16, 64, points_dict, [])
    r64_s_game2 = sim_game(seton_hall, tcu, 64, points_dict, [])
    r64_s_game3 = sim_game(houston, uab, 64, points_dict, [])
    r64_s_game4 = sim_game(illinois, chattanooga, 64, points_dict, [])
    r64_s_game5 = sim_game(colorado_state, michigan, 64, points_dict, [])
    r64_s_game6 = sim_game(tennessee, longwood, 64, points_dict, [])
    r64_s_game7 = sim_game(ohio_state, loyola_chicago, 64, points_dict, [])
    r64_s_game8 = sim_game(villanova, delaware, 64, points_dict, [])
